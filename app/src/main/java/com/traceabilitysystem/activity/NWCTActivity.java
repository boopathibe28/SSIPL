...
import com.traceabilitysystem.databinding.ActivityNwctCutterBinding;
...
public class NWCTActivity  extends AppCompatActivity implements View.OnClickListener {
    ActivityNwctCutterBinding binding;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_nwct_cutter);
    ...
    private void initialView() {
        ...
        binding.tvTitle.setText("NWCT");
        binding.edtMachineName.setText("NWCT");
    ...
    public void onClick(View view) {
        if (view == binding.imgBack){
            ...
            CommonFunctions.getInstance().newIntent(NWCTActivity.this, ScannedBarcodeActivity.class, bundle, false, false);
        }
            ...
            CommonFunctions.getInstance().newIntent(NWCTActivity.this, ScrapEnterCRCutterActivity.class, Bundle.EMPTY, false, false);
        }
        else if (view == binding.txtOk){
            if (binding.edtSSIPLID.getText().toString().trim().isEmpty()){
                CommonFunctions.getInstance().validationError(NWCTActivity.this,"SSIPL ID is Empty");
            }
            else if (binding.edtWorkOrderTrimmed.getText().toString().trim().isEmpty()){
                CommonFunctions.getInstance().validationError(NWCTActivity.this,"Work Order is Empty");
            }
            else {
                checkJobProcess();
            }
    ...
    private void apiCall(String input) {
        CommonApiCalls.getInstance().hrsSlitting(NWCTActivity.this, input, new CommonCallback.Listener() {
            @Override
            public void onSuccess(Object body) {
                ...
                if (apiResponse.getResult().equals("1")) {
                    ...
                    CommonFunctions.getInstance().successResponseToast(NWCTActivity.this, apiResponse.getMsg());
                }
                else {
                    // binding.txtOk.setVisibility(View.GONE);
                    binding.txtNotOk.setVisibility(View.VISIBLE);
                    binding.edtUpdate.setText(apiResponse.getMachine());
                    CommonFunctions.getInstance().validationError(NWCTActivity.this, apiResponse.getMsg());
                }
            }

            @Override
            public void onFailure(String reason) {
                CommonFunctions.getInstance().validationEmptyError(NWCTActivity.this, reason);
            }
        });
